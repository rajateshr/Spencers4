...
public class Globles {
	//Pincode
	public static String preprod_pincode = "560066";
...
// Verification
	...
	public static String QA_FutureGroup_URL = "http://preprod.fbbonline.in/";
	public static String QA_FutureGroup_UserName = "";
	public static String QA_FutureGroup_Password = "";
	
	public static String Enter_pincode = "560066";
	...
	/**
	 * @author Vishal ,To capture Screenshot
	 *         browser on Grid Browser - Chrome, firefox, IE 11
	 * 
	 */
	public String captureScreen() throws IOException {
	    String path;
	        ...
	        path = "ScreenCapture /fbb" + source.getName();
		...
		static {
			...
			System.setProperty("webdriver.chrome.driver",
					directoryPath + "/lib/DriverRelated/Chrome/chromedriver");
...
public static boolean isChecked(WebElement checkBox) {
	...
	{
		log(checkBox+ "Checkbox is selected successfully.=PASS");
	    return true;

	}
	else
	{
		log(checkBox+ "Checkbox is not selected=FAIL");
    return false;
	}
}
...
// Click a link using text passed from the test case
	...
	public static void enterText(WebElement TextElement,String text) {
		try{
			(TextElement).sendKeys(text);
		    log(text+" entered successfully=PASS");
	     }
		catch(Exception e){
			log("Fail to enter "+text+ " entered successfully=FAIL");
		}
	}
	...
	public static void isAttributePresent(WebElement element,String attrvalue) 
	{
	    if(element.getAttribute("value").equalsIgnoreCase(attrvalue))
	    		{
			log(attrvalue+" is present successfully=PASS");

	    		}
	    else
	    {
			log(attrvalue+" is not present=FAIL");
	...
	public static void moveToElement(WebElement elementToMoveOnto) {
		...
		if (elementToMoveOnto != null) {
			...
			actionsMove.moveToElement(elementToMoveOnto);
			actionsMove.perform();
	...
	public static boolean verifyElementIsPresent(WebDriver driver, WebElement webElement){
		  try {
		   ...
		   log("Element is displayed in webpage: " + webElement+ "=PASS");
		   return true;
		  } catch(NoSuchElementException e)   {
		   log("Unable to Displayed WebElement: " + webElement+ "=FAIL");
		   return false;
		  }
		 }
	...
	public static void scrollBy(int scrollHeight) {
		JavascriptExecutor js = ((JavascriptExecutor) driver);
	...
	public static void selectByValue(WebElement element, String inputData) {
		  try {
		   ...
		   log("Unable to select selectbox: " + element);
		  }
		 }
	...
	/*
	 *            selection selectFromListBox. selects the selection from the
	 *            list box
	 * 
	 *///
	...
	public void setUp() throws Exception {
		...
		if(!testMethodName.contains("DB") && !testMethodName.contains("DATABASE") &&
				!testMethodName.contains("FTP") && !testMethodName.contains("REST") &&
				!testMethodName.contains("SAUCE") && !testMethodName.contains("MOBILE")){*/
			browserSelect();
			browserMax();
		}
	...
	public static String switchToNextTabOrWindow() {
		...
		driver.switchTo().window(getWindowHandles().toArray()[Math.max(getWindowHandles().size() - 1, pos + 1)].toString());
		return driver.getTitle();
	}
	...
	/**
	 * @author Charan browserSelectGrid. Presents capabilities to select
	 *         browser on Grid Browser - Chrome, firefox, IE 11
	 * 
	 */
